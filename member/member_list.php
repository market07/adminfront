            ...
            foreach ($data as $val) {
                ...
                <tr class="center" data-member-no="<?= $val['memNo']; ?>">
                    ...
                    <td>
                        ...
                        <!--// ########## 튜닝 : 네이버 아이콘 처리 -->
                        <?php if($val['naver']){ ?>
                        <img src="/admin/image/naver_simple.png" alt="" style="width:12px;"/>
                        <?php } else if( $val['kakao'] ){ ?>
                        <img src="/admin/image/kakao_simple.png" alt="" style="width:14px;"/>
                    ...
                    <td>
                        ...
                        <?php if ($val['mallSno'] == 1) { ?>
                            <button type="button" class="btn btn-white btn-sm btnSendSms">SMS
                            </button>
                        <?php } ?>
                    </td>
                    <td>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('.btnSendSms', $formList).on('click', function (e) {
            if (member.get_member_attribute(e, 'data-cellPhone').length < 1) {
                alert('SMS 정보가 없는 회원입니다.');
                return;
            }
            if (member.get_member_attribute(e, 'data-smsFl') === 'y') {
                member_sms(member.get_member_attribute(e), member.get_member_attribute(e, 'data-memNm'), member.get_member_attribute(e, 'data-cellPhone'));
            } else {
                dialog_confirm(msg.DENY_SMS, function (result) {
                    if (result) {
                        member_sms(member.get_member_attribute(e), member.get_member_attribute(e, 'data-memNm'), member.get_member_attribute(e, 'data-cellPhone'));
                    } else {
                        layer_close();
