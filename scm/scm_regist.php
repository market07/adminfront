    ...
    <table class="table table-cols">
		...
		<tr>
            ...
            <td class="form-inline">
               ...
               카테고리 수수료&nbsp; <input type="radio" name="supplierType" value="B" <?= ($getData['supplierType'] != 'A' && $getData['supplierType'] != 'C') ? 'checked' : "";  ?>/>&nbsp;
			...
			<script>
				$(document).on('click','input[name=supplierType]',function(){
					var rval = $(this).val();
					if(rval == 'B'){
						$('input[name=scmProCom]').attr('disabled',true);
						$('input[name=scmProCom]').val('0');
					}else{
						$('input[name=scmProCom]').attr('disabled',false);
					}
				})
			</script>
        </tr>
