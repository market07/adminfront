...
<form id="frmSearchGoods" name="frmSearchGoods" method="get" class="js-form-enter-submit">
    ...
    <div class="search-detail-box">
        ...
        <table class="table table-cols">
			...
			<!-- lkm 튜닝 중복상품 검색 개발 방향 바뀌어 안씀 2018-07-09-->
			<tr>
                <th>판매상태에 따른 중복 상품 검색</th>
                <td colspan="3" style="/*background:#B7F0B1;*/">
                    <div class="form-inline">
						<label class="radio-inline">
							<input type="radio" name="overlap" value="" <?php if(!\Request::get()->get('overlap')){?>checked<?php }?>/>전체
						</label>
						<label class="radio-inline" style="/*background:#FAED7D;*/">
							<input type="radio" name="overlap" value="1" <?php if(\Request::get()->get('overlap') == 1){?>checked<?php }?>/>승인요청중복
						</label>
						<label class="radio-inline" style="/*background:#B7F0B1;*/"/>
							<input type="radio" name="overlap" value="2" <?php if(\Request::get()->get('overlap') == 2){?>checked<?php }?>>기존상품+승인상품+상품중복
						</label>
						<label class="radio-inline">
							<input type="radio" name="overlap" value="3" <?php if(\Request::get()->get('overlap') == 3){?>checked<?php }?>/>중복없음
						</label>
                    </div>
                </td>
            </tr>
