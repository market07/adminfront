...
for($i=0;$i<=10;$i++){
	if($data['goodsUnitPrice'.$i] == '0'){
		$data['goodsUnitPrice'.$i] = '';
	}
}
?>
<script type="text/javascript">
    ...
    $(document).ready(function () {
		...
		goodUnitPrices();
		$("input[name='optionFl']").on('click',function(){
			goodUnitPrices();
		});
        ...
        function unitPriceValidation()
        {
			
			for(i = 10 ; i>=1;i--)
            {
                     var val = parseInt($("#tbl_option_info_1 .optionGoodsUnitprice"+i).val());
                     var j = i-1;
					 var val1 = parseInt($("#tbl_option_info_1 .optionGoodsUnitprice"+j).val());
					 if( val1 !== 'Nan' && val>val1)
					 {
						 alert("Option Unit Price should be less.")
						 return false;
					 }
            }
			
			var fieldID = 'unitPrice';
            ...
            for(i = fieldNo ; i>=1;i--)
            {
                     var val = parseInt($("input[name='goodsUnitPrice"+i+"']").val());
                     if(val == 0)
                     {
                        check = 1;
                        break;
                     ...
                     else
                    {
                        ...
                        {
                             var j = i-1;
                             var val1 = parseInt($("input[name='goodsUnitPrice"+j+"']").val());
                             if(val>val1)
                             {
                                 check = 2;
                                 break;
            ...
            if(check == 1)
            {
                alert("Unit Price item should be greater than 0");
                return false;
            }
            else if(check == 2)
            {
                alert("Unit Price should be less");
                return false;
            }
            else
            {
        ...
        $('#btn_category_select').click(function () {
            ...
            for (var i = 0; i <= <?=DEFAULT_DEPTH_CATE;?>; i++) {
                if ($('#cateGoods' + i).val()) {
                    ...
                    if ($("#cateGoodsInfo" + cate.val()).length == 0) {
						...
						var percentage = cate.data('percentage');
						var naverid = cate.data('naverid');
                        ...
                        <?php } ?>
                        addHtml += "<td class='center'><input type='hidden' name='link[cateCd][]' value='" + cate.val() + "'><input type='hidden' name='link[cateLinkFl][]' value='y' id='cateLink_" + cate.val() + "'><input type='radio' name='cateCd' class='catSelect' value='" + cate.val() + "' data-percentage='"+percentage+"' data-naverid='"+naverid+"'></td>";
                        addHtml += "<td>" + (cateName.join(' &gt; ')).replace('&gt;', '') + "</td>";
                        addHtml += "<td class='center'>" + cate.val() + "</td>";
                        addHtml += "<td class='center'><input type='button' class='btn btn-sm btn-white btn-icon-minus' onclick='field_remove(\"cateGoodsInfo" + cate.val() + "\")' value='삭제'></span></td>";
                        addHtml += "<tr>";
                        $("#cateGoodsInfo").append(addHtml);
			...
			var percentage = $("input[name='cateCd']:checked").data('percentage');
			var naverid = $("input[name='cateCd']:checked").data('naverid');
			<?php if (gd_use_provider() === true && isset($supplierType)) { ?> 
				var sup =  '<?= $supplierType; ?>';
				var per =  parseFloat('<?= $scmProCom; ?>');
				if(sup == 'C'){
					
				}else if(sup == 'A'){
					percentage= per + percentage;
					$('input[name=commission]').val(percentage);
				}else{
					$('input[name=commission]').val(percentage);
				}
			<?php }else{ ?>
				$('input[name=commission]').val(percentage);
			<?php } ?>
			$('input[name=naverCategory]').val(naverid);
    ...
    function option_setting(thisCnt) {
        ...
        if (fieldChk > 0) {
            ...
            for (var i = fieldCnt; i < thisCnt; i++) {
                ...
                addHtml += '<td><input type="text" id="option_optionName_' + i + '" name="optionY[optionName][]" value="" class="form-control width-md" placeholder="ex)사이즈"/></td>';
				// onblur="option_grid();" 
                ...
                addHtml += '<input type="text" id="option_optionValue_' + i + '_0" data-option-sno="'+i+'" name="optionY[optionValue][' + i + '][]" value=""  class="form-control"  style="width:330px;"  placeholder="Enter키를 이용 옵션값을 연속적으로 입력하세요. ex)XL"  />';
				//onblur=" if(option_value_check(\'' + i + '\',\'0\') == true) { option_grid(); } "
     ...
     */
    function option_value_conf_remove(loc, locNo,NID) {
		
		var nvalue = jQuery('#'+NID).val();
		$( ".optValText" ).each(function( index ) {
			var nval = $( this ).val();

			nval = nval.split('^|^');
			if(nval.indexOf(nvalue) > -1){
				$(this).parent('tr').remove();
			}
		});
		
		var OptCnt = jQuery('#option_optionCnt_'+loc).val();
		if(parseInt(OptCnt) > 0){
			OptCnt = parseInt(OptCnt) - 1;
			jQuery('#option_optionCnt_'+loc).val(OptCnt);
		}
    ...
    function option_value_conf(loc, thisCnt, loadChk) {
        ...
        if (fieldChk > 0) {
            ...
            for (var i = fieldCnt; i < thisCnt; i++) {
                ...
                addHtml += '<input type="text" id="option_optionValue_' + loc + '_' + i + '" data-option-sno="'+loc+'" name="optionY[optionValue][' + loc + '][]" value="" class="form-control" style="width:330px;" placeholder="Enter키를 이용 옵션값을 연속적으로 입력하세요. ex)XL"  />';
				//onblur=" if(option_value_check(\'' + loc + '\',\'' + i + '\') == true) { option_grid(); } "
                addHtml += ' <input type="button" class="btn btn-sm btn-white btn-icon-minus " onclick="option_value_conf_remove(\'' + loc + '\',\'' + i + '\',\'option_optionValue_' + loc + '_' + i + '\');field_remove(\'optVal_' + loc + '_' + i + '\');" value="삭제" /> ';
    ...
    function option_grid(manualFl) {
		...
		// jQuery('#' + fieldTable+' tbody tr').each(function(tr){
			// console.log($(this).find('input[name="optionY[optionValueText][]"]').val());
		// });
		...
		//console.log(valCnt);return false;
        // 옵션 값 체크 설정
        ...
        for (var j = 0; j <= optTotCnt; j++) {
            if (j == 0) {
                ...
                addHtml += '<th class="width10p">개당 가격</th>';
                ...
                addHtml += '<th class="center"><div class="form-inline"><?=gd_currency_symbol();?><input type="text" id="option_opotionPriceApply" value="'+ $("input[name='goodsUnitPrice1']").val() + '" readonly class="form-control width-2xs" /><?=gd_currency_string();?></div></td>';
				addHtml += '<th class="center"><div class="form-inline"><input type="text" id="option_optionGoodsUnitprice1" class="form-control width-xs" /></div></th>';
                ...
                addHtml += '<tr id="tbl_option_info_' + j + '" class="mainOption">';
                ...
                addHtml += '<input type="hidden" name="optionY[optionValueText][]"  class="optValText" value="'+arrOption.join("<?=STR_DIVISION?>")+'" />';
                addHtml += '<td class="center"><div class="form-inline"><?=gd_currency_symbol();?><input type="text" id="option_optionCostPrice_' + optKey2 + '" name="optionY[optionCostPrice][]"   class="form-control width-2xs" /><?=gd_currency_string();?></div></td>';
                addHtml += '<td class="center"><div class="form-inline"><?=gd_currency_symbol();?><input type="text" id="option_optionPrice_' + optKey2 + '" name="optionY[optionPrice][]" value="'+ $("input[name='goodsUnitPrice1']").val() + '" readonly class="form-control width-2xs" /><?=gd_currency_string();?></div></td>';
				addHtml += '<td class="center"><div class="form-inline"><input type="text" id="option_optionGoodsUnitprice1_' + optKey2 + '" name="optionY[optionGoodsUnitprice1][]" value="" class="form-control width-xs optionGoodsUnitprice1" /><input type="text" id="option_optionGoodsUnitprice2_' + optKey2 + '" name="optionY[optionGoodsUnitprice2][]" value="" class="form-control width-xs optionGoodsUnitprice2" /><input type="text" id="option_optionGoodsUnitprice3_' + optKey2 + '" name="optionY[optionGoodsUnitprice3][]" value="" class="form-control width-xs optionGoodsUnitprice3" /><input type="text" id="option_optionGoodsUnitprice4_' + optKey2 + '" name="optionY[optionGoodsUnitprice4][]" value="" class="form-control width-xs optionGoodsUnitprice4" /><input type="text" id="option_optionGoodsUnitprice5_' + optKey2 + '" name="optionY[optionGoodsUnitprice5][]" value="" class="form-control width-xs optionGoodsUnitprice5" /><input type="text" id="option_optionGoodsUnitprice6_' + optKey2 + '" name="optionY[optionGoodsUnitprice6][]" value="" class="form-control width-xs optionGoodsUnitprice6" /><input type="text" id="option_optionGoodsUnitprice7_' + optKey2 + '" name="optionY[optionGoodsUnitprice7][]" value="" class="form-control width-xs optionGoodsUnitprice7" /><input type="text" id="option_optionGoodsUnitprice8_' + optKey2 + '" name="optionY[optionGoodsUnitprice8][]" value="" class="form-control width-xs optionGoodsUnitprice8" /><input type="text" id="option_optionGoodsUnitprice9_' + optKey2 + '" name="optionY[optionGoodsUnitprice9][]" value="" class="form-control width-xs optionGoodsUnitprice9" /><input type="text" id="option_optionGoodsUnitprice10_' + optKey2 + '" name="optionY[optionGoodsUnitprice10][]" value="" class="form-control width-xs optionGoodsUnitprice10" /></div></td>';
                ...
                addHtml += '</tr>';
				addHtml += '<tr data-for="' + j + '"><td colspan="' + (parseInt(optionCnt) + 7) + '"><input type="text" class="optAmount' + j + '" disabled/> &nbsp;<input type="radio" class="btn btn-sm btn-white optionRed" name="optionVal' + j + '" value="%" data-for="' + j + '"/> &nbsp;% 단위 할인  <input type="radio" class="btn btn-sm btn-white optionRed" name="optionVal' + j + '" value="kr" data-for="' + j + '"/> &nbsp;원 단위 할인 <input type="radio" class="btn btn-sm btn-white optionRed" name="optionVal' + j + '" value="m" data-for="' + j + '" checked/> &nbsp;직접 입력</td></tr>';
            }
        }
		
		
		addHtml += '<tr data-for="all"><td colspan="' + (parseInt(optionCnt) + 7) + '"></td></tr>';
		
        addHtml += '<tfoot><tr><td colspan="' + (parseInt(optionCnt) + 7) + '"><input type="button" class="btn btn-sm btn-white btn-icon-minus" onclick="delete_option(\'optionY[optionNo][]\',\'tbl_option_info_\');" value="선택 삭제" /></td></tr> <tr><td colspan="9">  <input type="radio" class="btn btn-sm btn-white" name="ismaster" value="master" data-for="master" checked/> &nbsp;일괄 할인 적용  <input type="radio" class="btn btn-sm btn-white" name="ismaster" value="nomaster" data-for="nomaster"/> &nbsp;일괄 할인 적용 안함 &nbsp;&nbsp;&nbsp;<input type="text" class="alloptAmount"/> &nbsp;<input type="radio" class="btn btn-sm btn-white alloptionRed" name="optionVal" value="%" data-for="all"/> &nbsp;% 단위 할인  <input type="radio" class="btn btn-sm btn-white alloptionRed" name="optionVal" value="kr" data-for="all"/> &nbsp;원 단위 할인 <input type="radio" class="btn btn-sm btn-white alloptionRed" name="optionVal" value="m" data-for="all" checked/> &nbsp;직접 입력</td></tr></tfoot>';
        addHtml += '</table>';
	...
	$(document).on('click','input[name=ismaster]',function(){
		var dataFor1 = $('input[name=ismaster]:checked').val();
		if(dataFor1 == 'master'){
			$('.alloptAmount').prop('disabled',false);
			for(var i=0;i<=10;i++){
				$('.optAmount'+i).prop('disabled',true);
			}
			
		}else{
			$('.alloptAmount').prop('disabled',true);
			for(var i=0;i<=10;i++){
				$('.optAmount'+i).prop('disabled',false);
			}
		}
	});
	
	$(document).on('click','.alloptionRed',function(){
		var dataFor1 = $('input[name=ismaster]:checked').val();
		if(dataFor1 == 'master'){
			var dataVal = $(this).val();
			var optAmount = $('.alloptAmount').val();
			var optCount = $('.mainOption').length;
			for(var dataFor=1;dataFor<=optCount;dataFor++){
				var option1 = $('#tbl_option_info_'+dataFor+' .optionGoodsUnitprice1').val();
				if(option1 == ''){
					alert("Please enter optionunitprice1 for option"+dataFor+".");
					$(this).prop('checked',false);
					return false;
				}else if(optAmount == '' && dataVal != 'm'){
					alert("Please enter decrease amount for option"+dataFor+".");
					$(this).prop('checked',false);
					return false;
				}
				if(dataVal === '%'){
					for(var i=2;i<=10;i++){
						var option1 = option1 - (parseFloat(option1) * (parseFloat(optAmount)/100)).toFixed(2);
						$('#tbl_option_info_'+dataFor+' .optionGoodsUnitprice'+i).val(Math.round(option1) - (Math.round(option1)%10));
						$('#tbl_option_info_'+dataFor+' .optionGoodsUnitprice'+i).attr('readonly','readonly');
					}
				}else if(dataVal === 'kr'){
					for(var i=2;i<=10;i++){
						var option1 = (parseFloat(option1) - (parseFloat(optAmount))).toFixed(2);
						var option1 = option1 > 0 ? option1 : 0;
						$('#tbl_option_info_'+dataFor+' .optionGoodsUnitprice'+i).val(Math.round(option1) - (Math.round(option1)%10));
						$('#tbl_option_info_'+dataFor+' .optionGoodsUnitprice'+i).attr('readonly','readonly');
					}
				}else{
					for(var i=2;i<=10;i++){
						$('#tbl_option_info_'+dataFor+' .optionGoodsUnitprice'+i).removeAttr('readonly');
					}
				}
			}
			
		}else{
			alert('일괄 할인 적용 안함 is enabled. Please select 일괄 할인 적용.');
			$(this).prop('checked',false);
			return false;
		}
	})
	
	$(document).on('click','.optionRed',function(){
		var dataFor1 = $('input[name=ismaster]:checked').val();
		if(dataFor1 != 'master'){
			var dataFor = $(this).closest('tr').attr('data-for');
			var dataVal = $(this).val();
			var optAmount = $('.optAmount'+dataFor).val();
			var option1 = $('#tbl_option_info_'+dataFor+' .optionGoodsUnitprice1').val();
			if(option1 == '' && dataVal != 'm'){
				alert("Please enter optionunitprice1 for option"+dataFor+".");
			...
			alert('일괄 할인 적용 is enabled. Please select 일괄 할인 적용 안함.');
			$(this).prop('checked', false).removeAttr('checked');
			return false;
		}
	})
    ...
    function option_value_apply() {
        ...
        if($('input[name="optionY[optionNo][]"]:checked').length) {
            ...
            var optionGoodsUnitprice1 = $('#option_optionGoodsUnitprice1').val();
            var optionGoodsUnitprice2 = $('#option_optionGoodsUnitprice2').val();
            var optionGoodsUnitprice3 = $('#option_optionGoodsUnitprice3').val();
            var optionGoodsUnitprice4 = $('#option_optionGoodsUnitprice4').val();
            var optionGoodsUnitprice5 = $('#option_optionGoodsUnitprice5').val();
            var optionGoodsUnitprice6 = $('#option_optionGoodsUnitprice6').val();
            var optionGoodsUnitprice7 = $('#option_optionGoodsUnitprice7').val();
            var optionGoodsUnitprice8 = $('#option_optionGoodsUnitprice8').val();
            var optionGoodsUnitprice9 = $('#option_optionGoodsUnitprice9').val();
            var optionGoodsUnitprice10 = $('#option_optionGoodsUnitprice10').val();
            ...
            $('input[name="optionY[optionNo][]"]').each(function (i) {
                if (this.checked) {
					...
					if (optionGoodsUnitprice1) {
                        $('input[name*=\'optionY\[optionGoodsUnitprice1\]\']').eq(i).val(optionGoodsUnitprice1);
                    }if (optionGoodsUnitprice2) {
                        $('input[name*=\'optionY\[optionGoodsUnitprice2\]\']').eq(i).val(optionGoodsUnitprice2);
                    }if (optionGoodsUnitprice3) {
                        $('input[name*=\'optionY\[optionGoodsUnitprice3\]\']').eq(i).val(optionGoodsUnitprice3);
                    }if (optionGoodsUnitprice4) {
                        $('input[name*=\'optionY\[optionGoodsUnitprice4\]\']').eq(i).val(optionGoodsUnitprice4);
                    }if (optionGoodsUnitprice5) {
                        $('input[name*=\'optionY\[optionGoodsUnitprice5\]\']').eq(i).val(optionGoodsUnitprice5);
                    }if (optionGoodsUnitprice6) {
                        $('input[name*=\'optionY\[optionGoodsUnitprice6\]\']').eq(i).val(optionGoodsUnitprice6);
                    }if (optionGoodsUnitprice7) {
                        $('input[name*=\'optionY\[optionGoodsUnitprice7\]\']').eq(i).val(optionGoodsUnitprice7);
                    }if (optionGoodsUnitprice8) {
                        $('input[name*=\'optionY\[optionGoodsUnitprice8\]\']').eq(i).val(optionGoodsUnitprice8);
                    }if (optionGoodsUnitprice9) {
                        $('input[name*=\'optionY\[optionGoodsUnitprice9\]\']').eq(i).val(optionGoodsUnitprice9);
                    }if (optionGoodsUnitprice10) {
                        $('input[name*=\'optionY\[optionGoodsUnitprice10\]\']').eq(i).val(optionGoodsUnitprice10);
                    }
    ...
    function add_item_price()
    {
        ...
        var fieldNo = parseInt(fieldNoChk) + 1;
        var att = parseInt(fieldNo) + 1;
        var addHtml = '';
        addHtml += '<tr id="' + fieldID + fieldNo + '">';
        addHtml += '<th>'+att+'개 구매할 때 객단가</th>';
        addHtml += '<td><input type ="text" name="goodsUnitPrice'+att+'" value="<?php  echo gd_money_format($data["goodsUnitPrice'+att+'"], false); ?>"" class="" maxlength="250"/>';      
        addHtml += ' <td></tr>';
        $('#' + fieldID).append(addHtml);
        if(att == 10)
        {
            $("#unit_price_button").remove();
        }
		

    }

     $(document).on('change', 'input[name="goodsPrice"]', function() {  
        $("input[name='goodsUnitPrice1']").val($(this).val());
        $("input[name='optionY[optionPrice][]']").val($(this).val());
    });
    $(document).on('change', 'input[name="goodsUnitPrice1"]', function() {  
        $("input[name='goodsPrice']").val($(this).val());
        $("input[name='optionY[optionPrice][]']").val($(this).val());
    });
    ...
    $(document).ready(function () {
         <?php
            ...
            { 
               ?>
                    var tempPrice = "<?php  echo $data['goodsUnitPrice'.$i] ?>";
                    ...
                    //{
                        ...
                        addHtml += '<tr id="' + fieldID + fieldNo + '">';
                        addHtml += '<th>'+att+'개 구매할 때 객단가</th>';
                        addHtml += '<td><input type ="text" name="goodsUnitPrice'+att+'" value="'+tempPrice+'" class="" maxlength="250"/>';      
                        addHtml += ' <td></tr>';
                        $('#' + fieldID).append(addHtml);
                        if(att == 10)
                        {
        ...
        ?>
		var fieldID = 'unitPrice';
		addHtml = '<tr><td><input type="text" class="wooptAmount"/></td><td><input type="radio" class="btn btn-sm btn-white wooptionRed" name="wooptionVal" value="%"/> &nbsp;% 단위 할인  <input type="radio" class="btn btn-sm btn-white wooptionRed" name="wooptionVal" value="kr"/> &nbsp;원 단위 할인 <input type="radio" class="btn btn-sm btn-white wooptionRed" name="wooptionVal" value="m" checked/> &nbsp;직접 입력</td></tr>';
		
		$('#unitPrice').append(addHtml);


		$(document).on('click','.wooptionRed',function(){
			var dataFor = $(this).closest('tr').attr('data-for');
			var dataVal = $(this).val();
			var optAmount = $('.wooptAmount').val();
			var option1 = $('#unitPrice #goodsUnitPrice1').val();
			if(option1 == ''){
				alert("Please enter goodsUnitPrice1.");
				$(this).prop('checked',false);
				return false;
			}else if(optAmount == ''){
				alert("Please enter decrease amount.");
				$(this).prop('checked',false);
				return false;
			}
			if(dataVal === '%'){
				for(var i=2;i<=10;i++){
					var option1 = option1 - (parseFloat(option1) * (parseFloat(optAmount)/100)).toFixed(2);
					$('#unitPrice input[name=goodsUnitPrice'+i+']').val(Math.round(option1) - (Math.round(option1)%10));
					$('#unitPrice input[name=goodsUnitPrice'+i+']').attr('readonly','readonly');
				}
			}else if(dataVal === 'kr'){
				for(var i=2;i<=10;i++){
					var option1 = (parseFloat(option1) - (parseFloat(optAmount))).toFixed(2);
				...
				for(var i=2;i<=10;i++){
					$('#unitPrice input[name=goodsUnitPrice'+i+']').removeAttr('readonly');
				}
			}
	});		
    });
...
<form id="frmGoods" name="frmGoods" action="./goods_ps.php" method="post" enctype="multipart/form-data">
    ...
    <div class="page-header js-affix">
        ...
        <div class="btn-group">
            ...
            <?php } else { ?>
                <input type="button" value="목록" class="btn btn-white btn-icon-list" onclick="goList('./goods_list.php');" />
                <?php if ($data['mode'] == 'register') { ?>
                ...
                <td <?php if (gd_use_provider() !== true || gd_is_provider() !== false) { ?>colspan="3" <?php } ?>>
                    <div class="form-inline">
                        <label title="수수료를 입력해 주세요!">
                            <input type="text" name="commission" value="<?=$data['commission']; ?>" class="form-control width50p" onchange="setCommissionPrice()" <?php if (gd_use_provider() !== true || gd_is_provider() !== false) { ?>disabled <?php } ?>/>&nbsp;%
                        </label>
                    </div>
                </td>
            </tr>
    ...
    <div id="depth-toggle-layer-stockOption">
        <table class="table table-cols">
            ...
            <tr>
                ...
                <td>
                    <div class="notice-info">
                        ...
                        옵션명/옵션값 설정후에 자동으로 옵션의 가격 설정부분이 나오게 됩니다. 
                    </div>
                    <div id="option"></div>
					<div class="form-inline">
					<input type="button" class="btn btn-sm btn-gray" onclick="option_grid('y');" value="옵션 가격 설정 적용">
					&nbsp;<input type="button" class="btn btn-sm btn-gray" onclick="option_grid();" value="Reset">
                    </div>
                    <div id="optionGrid" style="width: 100%;overflow-x: scroll;">
                        <?php
                        if (!empty($data['option']) && $data['optionCnt'] > 0) {
                            ...
                            <table id="optionGridTable" class="table table-cols">
                                ...
                                <tr>
									...
									<th class="center">
                                        <div class="form-inline">
                                            <input type="text" id="option_optionGoodsUnitprice1" class="form-control width-xs"/>
                                        </div>
                                    </th>
                                    <th class="center">
                                        <div class="form-inline"><?=gd_currency_symbol(); ?>
                                ...
                                foreach ($data['option'] as $key => $val) {
                                    $nextNo++;
                                    ?>
                                    <tr id="tbl_option_info_<?=$key+1?>" class="mainOption">
                                        ...
                                        <input type="hidden" name="optionY[optionValueText][]" value="<?=implode(STR_DIVISION,$tmpOptionText)?>" class="optValText"/>
                                        <td class="center">
                                            <div class="form-inline">
                                        ...
                                        <td class="center">
                                            <div class="form-inline"><?=gd_currency_symbol(); ?>
                                                <input type="text" name="optionY[optionPrice][]" readonly value="<?=gd_money_format(gd_isset($data['option'][$key]['optionPrice']), false); ?>" class="form-control width-2xs"/><?=gd_currency_string(); ?>
                                            </div>
                                        </td>
										<td class="center">
                                            <div class="form-inline">
                                                <input type="text" name="optionY[optionGoodsUnitprice1][]" value="<?=gd_isset($data['option'][$key]['optionGoodsUnitprice1']); ?>" class="form-control width-xs optionGoodsUnitprice1"/>
												<input type="text" name="optionY[optionGoodsUnitprice2][]" value="<?=gd_isset($data['option'][$key]['optionGoodsUnitprice2']); ?>" class="form-control width-xs optionGoodsUnitprice2"/>
												<input type="text" name="optionY[optionGoodsUnitprice3][]" value="<?=gd_isset($data['option'][$key]['optionGoodsUnitprice3']); ?>" class="form-control width-xs optionGoodsUnitprice3"/>
												<input type="text" name="optionY[optionGoodsUnitprice4][]" value="<?=gd_isset($data['option'][$key]['optionGoodsUnitprice4']); ?>" class="form-control width-xs optionGoodsUnitprice4"/>
												<input type="text" name="optionY[optionGoodsUnitprice5][]" value="<?=gd_isset($data['option'][$key]['optionGoodsUnitprice5']); ?>" class="form-control width-xs optionGoodsUnitprice5"/>
												<input type="text" name="optionY[optionGoodsUnitprice6][]" value="<?=gd_isset($data['option'][$key]['optionGoodsUnitprice6']); ?>" class="form-control width-xs optionGoodsUnitprice6"/>
												<input type="text" name="optionY[optionGoodsUnitprice7][]" value="<?=gd_isset($data['option'][$key]['optionGoodsUnitprice7']); ?>" class="form-control width-xs optionGoodsUnitprice7"/>
												<input type="text" name="optionY[optionGoodsUnitprice8][]" value="<?=gd_isset($data['option'][$key]['optionGoodsUnitprice8']); ?>" class="form-control width-xs optionGoodsUnitprice8"/>
												<input type="text" name="optionY[optionGoodsUnitprice9][]" value="<?=gd_isset($data['option'][$key]['optionGoodsUnitprice9']); ?>" class="form-control width-xs optionGoodsUnitprice9"/>
												<input type="text" name="optionY[optionGoodsUnitprice10][]" value="<?=gd_isset($data['option'][$key]['optionGoodsUnitprice10']); ?>" class="form-control width-xs optionGoodsUnitprice10"/>
                                            </div>
                                        </td>
									...
									<tr data-for="<?=$key+1?>"><td colspan="<?=$data['optionCnt']+7?>"><input type="text" class="optAmount<?=$key+1?>" disabled/> &nbsp;<input type="radio" class="btn btn-sm btn-white optionRed" name="optionVal<?=$key+1?>" value="%" data-for="<?=$key+1?>"/> &nbsp;% 단위 할인  <input type="radio" class="btn btn-sm btn-white optionRed" name="optionVal<?=$key+1?>" value="kr" data-for="<?=$key+1?>"/> &nbsp;원 단위 할인 <input type="radio" class="btn btn-sm btn-white optionRed" name="optionVal<?=$key+1?>" value="m" data-for="<?=$key+1?>" checked/> &nbsp;직접 입력</td></tr>
                                    <?php
                                }
                                ?>
                                <tfoot>
                                <tr>
								 ...
								 <tr><td colspan="9">  <input type="radio" class="btn btn-sm btn-white" name="ismaster" value="master" data-for="master" checked/> &nbsp;일괄 할인 적용  <input type="radio" class="btn btn-sm btn-white" name="ismaster" value="nomaster" data-for="nomaster"/> &nbsp;일괄 할인 적용 안함 &nbsp;&nbsp;&nbsp;<input type="text" class="alloptAmount"/> &nbsp;<input type="radio" class="btn btn-sm btn-white alloptionRed" name="optionVal" value="%" data-for="all"/> &nbsp;% 단위 할인  <input type="radio" class="btn btn-sm btn-white alloptionRed" name="optionVal" value="kr" data-for="all"/> &nbsp;원 단위 할인 <input type="radio" class="btn btn-sm btn-white alloptionRed" name="optionVal" value="m" data-for="all" checked/> &nbsp;직접 입력</td></tr>
                                </tfoot>
                            </table>
                            <?php
                        }
          ...
          <div class="table-title gd-help-manual unitpricediv">
        Unit Price
            <table id="unitPrice" class="table table-cols">
                <colgroup>
                    <col class="width-lg"/>
                    <col class="width-2xl"/>
                    <col class="width-md"/>
                   ...
                   <tr id="unitPrice0">
                    ...
                    <td>
                        <span>
                        <input type ="text" id="goodsUnitPrice1" name="goodsUnitPrice1" value="<?=gd_money_format($data['goodsUnitPrice1'], false); ?>"  maxlength="250"/>
                        <input type='button' id ="unit_price_button" class='btn btn-sm btn-white btn-icon-plus' onclick='add_item_price()'>
                        </span>
                    <td>
                </tr>
            </table>
    ...
    <div id="depth-toggle-layer-sellInfo">
        <table class="table table-cols sales-information">
            ...
            <tr>
                ...
                <th>재입고 알림</th>
                <td>
                    <label class="radio-inline"><input type="radio" name="restockFl" value="y" <?=gd_isset($checked['restockFl']['y']); ?> />사용함</label>
                    <label class="radio-inline"><input type="radio" name="restockFl" value="n" <?=gd_isset($checked['restockFl']['n']); ?> />사용안함</label>
                </td> -->
            </tr>
            <tr>
                <th>판매기간</th>
    ...
    <?php if (gd_is_provider() === false) { ?>
        ...
        <div id="depth-toggle-layer-setMilage">
            <table class="table table-cols">
                <colgroup>
                    <col class="width-lg">
                    <col>
                ...
                <tr>
                    ...
                    <td>
                        <table class="table table-cols">
                            ...
                            <tr>
                                <th>
                                    <label class="radio-inline"><input type="radio" name="mileageFl" value="c" <?=gd_isset($checked['mileageFl']['c']); ?> onclick="display_toggle('mileageGoodsConfig','hide');display_toggle('mileageBasic','show');"/>통합설정</label>
                                </th>
                                <td>
                                    <div>구매 상품 금액의 <span><?=$conf['mileage']['goods'] ?>%</span>

                                        <?php
                                        if ($conf['mileageBasic']['goodsPrice'] == '1') $mileagePriceStr[] = "판매가";
                                        if ($conf['mileageBasic']['optionPrice'] == '1') $mileagePriceStr[] = "옵션가";
                                        if ($conf['mileageBasic']['addGoodsPrice'] == '1') $mileagePriceStr[] = "추가상품가";
                                        if ($conf['mileageBasic']['textOptionPrice'] == '1') $mileagePriceStr[] = "텍스트옵션가";
                                        ?>
                                        (<?= implode("+", $mileagePriceStr) ?>)

                                    </div>
                                    <div> - 기준 :
                                        <a href="../member/member_mileage_give.php" target="_blank" class="btn-link">회원>마일리지/예치금 관리>마일리지 지급설정</a>에서 설정한 기준에 따름
                                    </div>
                            ...
                            <tr>
                                <th>
                                    <label class="radio-inline"><input type="radio" name="mileageFl" value="g" <?=gd_isset($checked['mileageFl']['g']); ?> onclick="display_toggle('mileageGoodsConfig','show');display_toggle('mileageBasic','hide');"/>개별설정</label>
                                </th>
                                <td>
                                    <div id="mileageBasic">이 상품의 마일리지지급 기준을 따로 설정함</div>
                                    <div id="mileageGoodsConfig">
                                        <div class="form-inline">구매금액의
                                            <input type="text" name="mileageGoods" value="<?=$data['mileageGoods'] ?>" class="form-control width-sm">
                                            <select name="mileageGoodsUnit" class="form-control width-2xs">
                        ...
                        <div>
                        <span class="notice-info">절사기준 : <?=gd_trunc_display('mileage'); ?>
                            <a href="../policy/base_currency_unit.php" target="_blank" class="btn-link">기본설정>기본정책>금액/단위 기준설정</a>에서 설정한 기준에 따름</span>
                        </div>
        ...
        <div class="table-title gd-help-manual">
            상품 할인 설정 <span class="notice-info">상품 할인은 옵션가, 추가상품가, 텍스트옵션가 제외<span class="text-danger">판매가에만 적용</span>됩니다</span>
        ...
        <div id="depth-toggle-line-goodsDiscount" class="depth-toggle-line display-none"></div>
        <div id="depth-toggle-layer-goodsDiscount">
            <table class="table table-cols">
                <colgroup>
                    <col class="width-lg">
                    <col>
                ...
                <tr>
                    ...
                    <td>
                        <div class="radio">
                            <label class="radio-inline">
                                <input type="radio" name="goodsDiscountFl" value="n" onclick="display_toggle('goodsDiscountConfig','hide');" <?=gd_isset($checked['goodsDiscountFl']['n']); ?>>상품별 할인 사용안함
                            </label>
                            <label class="radio-inline">
                                <input type="radio" name="goodsDiscountFl" value="y" onclick="display_toggle('goodsDiscountConfig','show');" <?=gd_isset($checked['goodsDiscountFl']['y']); ?>>상품별 할인 사용함
                            </label>
                        </div>
                        <div id="goodsDiscountConfig" class="display-none"></diuv>
                            <div class="form-inline">
                                판매금액의
                                <input type="text" name="goodsDiscount" value="<?=gd_money_format($data['goodsDiscount'], false); ?>" class="form-control width-sm">
                                <select name="goodsDiscountUnit" class="form-control width-2xs">
                            ...
                            <span class="notice-info">절사기준 : <?=gd_trunc_display('goods'); ?>
                                <a href="../policy/base_currency_unit.php" target="_blank">[기본설정>기본정책>금액/단위 기준설정]</a>에서 설정한 기준에 따름</span>
                            </div>
                        </div>
                    </td>
                </tr>
            ...
            <table class="table table-cols">
                ...
                <tr>
                    ...
                    <td>
                        <div class="form-inline">
                            <?=gd_currency_symbol(); ?>
                            <input type="text" id="goodsPrice"  name="goodsPrice" value="<?=gd_money_format($data['goodsPrice'], false); ?>" class="form-control width-sm" onchange="setCommissionPrice()">
                            <?=gd_currency_string(); ?>
                        </div>
                    </td>
                    <th>공급가</th>
    ...
    <div id="depth-toggle-layer-naverEp">
        <table class="table table-cols">
            ...
            <tr>
                ...
                <td colspan="3">
					...
					<div class="col-md-6">
						...
						<div class="col-md-11">
							<input type="text" name="naverfee" value="<?=$data['naverfee']; ?>" class="form-control width-md"/>
						</div>
					</div>
                    <p class="notice-info" style="float: left;">네이버쇼핑 카테고리에 매칭할 수 있는 정보입니다. 카테고리 ID 정보는 <a class="btn-link" href="https://adcenter.shopping.naver.com/main.nhn" target="_blank">네이버쇼핑 쇼핑파트너존</a>에서 다운로드할 수 있습니다.</p>
                </td>
            </tr>
            <tr>
                <th>가격비교 페이지 ID</th>
            ...
            <tr>
                ...
                <td colspan="3">
                    <label>
                        <input type="text" name="crunchNaverPrice" value="<?=gd_money_format($data['crunchNaverPrice'], false); ?>" class="form-control width-md js-maxlength <?php if (gd_is_provider() === true) { ?>readonly<?php } ?>" maxlength="50"    <?php if (gd_is_provider() === true) { ?>readonly<?php } ?>/>
                    </label>
                </td>
            </tr>
        </table>
                        ...
                        foreach ($fbGoodsImage as $key => $iVal) {
                            if ($data['imageStorage'] == 'url') {
                                ...
                                if (strtolower(substr($iVal, 0, 4)) == 'http') {
                                    ...
                                    echo '	$(\'#imageFbGoodsPreView' . $imageNo . '\').html(\' ' . gd_htmlspecialchars_slashes($preViewImg, 'add') . ' : 가로 ' . $imageWidth . 'pixel(픽셀))\');' . chr(10);
                                    ...
                                    $preViewImg = $imageInput . gd_html_preview_image($iVal, $data['imagePath'], $data['imageStorage'], 25, 'goods', null, null, true, true, $imageSize) . $delHtml . '&nbsp;&nbsp;' . gd_htmlspecialchars_stripslashes(gd_htmlspecialchars_slashes($iVal, 'add'));
                                    echo '	$(\'#imageFbGoodsPreView' . $imageNo . '\').html(\' ' . gd_htmlspecialchars_slashes($preViewImg, 'add') . ' : 가로 ' . $imageWidth . 'pixel(픽셀)&nbsp;&nbsp;\');' . chr(10);
                                }
                                $imageNo++;
                            }
