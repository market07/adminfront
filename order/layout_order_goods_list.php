...
<div class="table-responsive">
    <table class="table table-rows order-list">
        ...
        <tr>
            ...
            <?php if(in_array($currentUserHandleMode, ['e', 'b', 'r'])) { ?>
            ...
            <?php if (!$isProvider) { ?>
                ...
                <?php } ?>
                <?php if(gd_is_plus_shop(PLUSSHOP_CODE_PURCHASE) === true) { ?><th class="width7p">매입처</th><?php } ?>
                <th class="width7p">사은품</th>
            <?php } ?>
            <?php if (!in_array(substr($currentStatusCode, 0, 1), ['d', 's'])) { ?>
            ...
            <?php if ($isUserHandle) { ?>
                ...
                <?php } elseif ($currentUserHandleMode == 'r') { ?>
                    <th class="width3p">환불수량</th>
                <?php } ?>
                <th class="width5p">메모</th>
            <?php } ?>
            <?php if (!$isProvider) { ?>
            <th class="width10p">관리자메모</th>
            <?php } ?>
        </tr>
        </thead>
        <tbody>
                    ...
                    foreach ($sVal as $dKey => $dVal) {
                        ...
                        foreach ($dVal as $key => $val) {
                            ...
                            <tr class="text-center" data-mall-sno="<?=$val['mallSno']?>" data-channel="<?=$val['orderChannelFl']?>">
                                ...
                                <?php } ?>
                                <td><a href="./order_view.php?orderNo=<?= $orderNo; ?>" target="_blank" title="주문번호" class="font-num<?=$isUserHandle ? ' js-link-order' : ''?>" data-order-no="<?=$orderNo?>" data-is-provider="<?= $isProvider ? 'true' : 'false' ?>"><?= $val['sno'] ?></a>
                                    <?php if ($val['orderChannelFl'] == 'naverpay') { ?>
                                ...
                                <?php if(in_array($currentUserHandleMode, ['e', 'b', 'r'])) {
                                echo '<td class="text-center">' . $orderStatusCodeByAdmin[$val['orderStatus']] . '</td>';
                                } ?>
                                ...
                                <?php } ?>
                                <?php if ($isUserHandle) { ?>
                                    <?php if ($val['userHandleGoodsCnt'] > 0) { ?>
                                    <td <?= $orderAddGoodsRowSpan; ?>><?=number_format($val['userHandleGoodsCnt'])?></td>
                                    <td <?= $orderAddGoodsRowSpan ?> data-order-no="<?= $val['orderNo'] ?>" data-handle-sno="<?= $val['userHandleNo'] ?>" data-status-mode="<?= $currentUserHandleMode ?>">
                                    <?php } else {?>
                                    <td <?= $orderAddGoodsRowSpan; ?>><?=number_format($val['goodsCnt'])?></td>
                                    <td <?= $orderAddGoodsRowSpan ?> data-order-no="<?= $val['orderNo'] ?>" data-handle-sno="<?= $val['userHandleSno'] ?>" data-status-mode="<?= $currentUserHandleMode ?>">
                                    <?php } ?>
                                        <p class="memo">
